...
class DatabaseManager {
  ...
  init() {
    ...
    // Always run schema migration before any SQL operations or data access
    try {
      this.migrateSchema();
    } catch (e) {
      console.warn('[DatabaseManager] Schema migration failed:', e);
    }

    // load any queued offline changes into memory
    this.pendingSync = JSON.parse(localStorage.getItem(this.dbName + '_sync') || '[]');

    // listen for connectivity restoration to flush queue
      ...
      if (window && window.electronAPI && window.electronAPI.queryDatabase) {
        ...
        window.electronAPI.queryDatabase("SELECT name FROM sqlite_master WHERE type='table' LIMIT 1").then((probe) => {
          if (probe && probe.success) {
            ...
            // ensure required columns exist in SQLite before any import
            // adding columns will fail harmlessly if they already exist
            if (window.electronAPI.queryDatabase) {
              window.electronAPI.queryDatabase('ALTER TABLE patients ADD COLUMN firstName TEXT').catch(() => {});
              window.electronAPI.queryDatabase('ALTER TABLE patients ADD COLUMN lastName TEXT').catch(() => {});
            }
  ...
  saveDatabase(db) {
    ...
    this.queueSync({ type: 'dbSnapshot', timestamp: Date.now(), data: db });
  ...
  migrateSchema() {
    ...
    if (db && Array.isArray(db.patients)) {
      ...
      db.patients = db.patients.map(p => {
        if (!('firstName' in p)) {
          // try to split existing name field
          const full = p.name || '';
      ...
      if (changed) {
        console.log('[DatabaseManager] Schema migration: added firstName/lastName to patients');
        this.saveDatabase(db);
      }
    }
    // additional migrations can be chained here in future
  }
  ...
  queueSync(item) {
    ...
    try {
      localStorage.setItem(this.dbName + '_sync', JSON.stringify(this.pendingSync));
    } catch (e) {
      console.warn('[DatabaseManager] failed to persist sync queue', e);
    }
  }

  flushSyncQueue() {
    if (!navigator.onLine) return;
    if (!this.pendingSync || this.pendingSync.length === 0) return;
    console.log('[DatabaseManager] flushing sync queue', this.pendingSync.length, 'items');
    // placeholder: emit each change to server or use electronAPI
    this.pendingSync.forEach(item => {
      if (window && window.electronAPI && window.electronAPI.syncChange) {
        window.electronAPI.syncChange(item);
      } else {
        // implement your own network call
        fetch('/api/sync', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(item) })
          .catch(err => console.warn('sync error', err));
      }
    });
    this.pendingSync = [];
    localStorage.removeItem(this.dbName + '_sync');

    // after successfully pushing, try to pull latest state from server
    if (window && window.electronAPI && window.electronAPI.fetchLatestData) {
      window.electronAPI.fetchLatestData().then((data) => {
        if (data) {
          console.log('[DatabaseManager] Received latest data from server, overwriting local DB');

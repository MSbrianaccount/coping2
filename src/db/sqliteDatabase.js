...
class SQLiteDatabase {
  ...
  init() {
    try {
      ...
      // Create schema if first run; otherwise ensure expected columns exist
      if (!this.tableExists('patients')) {
        this._createSchema();
      } else {
        this._ensureColumns('patients', ['firstName', 'lastName']);
        this._ensureColumns('staff', ['firstName', 'lastName']);
      }
  ...
  /**
   * Ensure a set of columns exist on a table; add any missing columns.
   */
  _ensureColumns(tableName, columns) {
    try {
      const existing = this.db.prepare(`PRAGMA table_info(${tableName})`).all();
      ...
      for (const col of columns) {
        if (!existingNames.includes(col)) {
          ...
          try {
            this.db.exec(sql);
            console.log(`[SQLite] Added missing column ${col} to ${tableName}`);
          } catch (e) {
            console.warn(`[SQLite] Failed to add column ${col} to ${tableName}: ${e.message}`);
          }
        }
      }
    } catch (err) {
      console.warn(`[SQLite] Could not ensure columns for ${tableName}: ${err.message}`);
    }
  }

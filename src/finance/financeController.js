...
class FinanceController {
  constructor() {
    ...
    const BillingClass = typeof BillingService !== 'undefined' ? BillingService : (window && window.BillingService ? window.BillingService : null);
    if (!BillingClass) {
      console.warn('[FinanceController] BillingService not available in browser; using mock object.');
      this.billingService = {
        calculateTotal: (services) => (Array.isArray(services) ? services.reduce((sum, s) => {
      ...
      this.billingService = new BillingClass();
    }
  }
